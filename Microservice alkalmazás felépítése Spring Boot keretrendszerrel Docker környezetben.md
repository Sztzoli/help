...
* Komponensek életciklus kezelése és kapcsolatok: nem a programozó példányosítja a komponenseket hanem a keretrendszer
* Távoli elérés: elérhető másik számítógép számára is tipikusan weben keresztül
* Többszálúság: keretrendszer biztosítka
* Perzisztencia: Java Se JDBC már elavult(checked exception)
* Tranzakció-kezelés:
...
![](C:\Users\BeneZ\Documents\jegyzetek\sbimage\spring-app-context.png)
...
![](C:\Users\BeneZ\Documents\jegyzetek\sbimage\spring-app-context-layers.png)
...
* Verziókezelés: "One codebase tracked in revision control, many deploys" : adott alkalmazáshoz tartozó forráskódok, erőforrás állományok egy repositoriba egy verzió kezelő alatt kell lenniük
* Függőségek: "Explicitly declare and isolate dependencies": alkalmazásnak külső függőségeit explicit módon deklarálni kell és külön kell ezeket tartani az alkalmazástól
* Konfiguráció: "Store config in the environment":  Adott környezetekre jellemző konfigurációk nem az alkalmazás részét képezik hanem adott környezeten kell lenni
* Háttérszolgáltatások: "Treat backing services as attached resources": külön kell kezelni ezek nem függőségek
* Build, release, futtatás: "Strictly separate build and run stages": külön kell venni a build, release, futtatás lépéseit
* Folyamatok: "Execute the app as one or more stateless processes": alkalmazás állapotmentes proccessekből áll
...
* Disposability: "Maximize robustness with fast startup and graceful shutdown" gyorsan induljon el és álljon le
* Éles és fejlesztői környezet hasonlósága: <br /> "Keep development, staging, and production as similar as possible" : ne legyen rajta eltérlés ha lehet
* Naplózás: "Treat logs as event streams" : fontos h naplozzon, konzolra 
...
![container-what-is-container](C:\Users\BeneZ\Documents\jegyzetek\sbimage\container-what-is-container.png)
...
![docker-containers](C:\Users\BeneZ\Documents\jegyzetek\sbimage\docker-containers.jpg)
...
```shell
docker version // docker verzió szám
docker run hello-world // elinditja a hello-world-et ha nem találja letölti a docker hub ról
docker run -p 8080:80 nginx // 8080 hostra vezetiki a konténer 80 portját
docker run -d -p 8080:80 nginx // visszakapjuk a promtot
docker ps //listáza a konténereket
docker stop 517e15770697 // leálítja a 517 conténert
docker run -d -p 8080:80 --name nginx nginx //--name nével hivatkozunk rá
docker stop nginx
docker ps -a // összes futó konténert ki listázza
docker start nginx //imigeból csinál konténert és elinditja
docker logs -f nginx //mit ir ki a konténer 
docker stop nginx // leállitja
docker rm nginx // törli a leállított konténert
```
...
```shell
docker images // listázza
docker rmi nginx // törli
```
...
```shell
docker build -t employees . /image neve employees legyen
docker run -d -p 8080:8080 employees /ezzel inditjuk el visszakapjuk a pronot 8080porton inditja el
```
...
![docker-layers](C:\Users\BeneZ\Documents\jegyzetek\sbimage\docker-layers.png)
...
git remote add origin https://github.com/username/employees.git // öszekötjük a lokált a távolival
git push -u origin master // pusholjuk
```
...
![rest-architecture](C:\Users\BeneZ\Documents\jegyzetek\sbimage\rest-architecture.png)
...
* PUT idempotens: többször elvégezünk a műveletet csak egyszer fog módosítást végre hajtodni
